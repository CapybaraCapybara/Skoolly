...
def fast_extract_coordinates_from_web(url, province=""):
    ...
    if not u.startswith("http"):
        u = "https://" + u
    try:
        with fast_session.get(u, timeout=1.5, stream=True, allow_redirects=True) as r:
            if r.status_code == 200:
                ...
                for chunk in r.iter_content(chunk_size=16384):
                    chunks.append(chunk)
                    total += len(chunk)
                    if total > 65536: # 64 KB is enough
                        break
...
def fast_geocode_precision(school_name_th, school_name_en, address, district, province, subdistrict="", web=""):
    ...
    # 1. Cleaned Street Address (ArcGIS - Fastest & Highest Hit Rate in Thailand)
    street_queries = []
    if cleaned_addr:
    ...
    for q in street_queries:
        ...
        try:
            r = fast_session.get(url, timeout=2.0)
            if r.status_code == 200:
    ...
    # 3. Website Map Embed (If web exists)
    if web:
        w_lat, w_lon = fast_extract_coordinates_from_web(web, province)
        if w_lat and w_lon:
            return w_lat, w_lon, "Website Map Embed", "Exact"

    # 4. Open-Meteo District / Subdistrict Fallback (Approximate Centroid)
...
def process_single(s):
    ...
    lat, lon, src, prec = fast_geocode_precision(th_name, en_name, address, d, p, sd, web)
    return s, lat, lon, src, prec

# Let's load the 72 schools that were enriched
with open('data/international_schools_thailand_opec.json', 'r', encoding='utf-8') as f:
    schools = json.load(f)

# Reset their coordinates to simulate the missing ones before enrichment
targets = [s for s in schools if is_imprecise_centroid(s.get('latitude'), s.get('longitude')) or s.get('gps_source', '').startswith('ArcGIS') or s.get('gps_source', '').startswith('District')]
print(f"Total test targets: {len(targets)}")
...
print(f"\n=======================================================")
print(f"Time Taken: {round(elapsed, 2)} seconds (Speed: {round(len(targets)/elapsed, 1)} schools/sec)")
