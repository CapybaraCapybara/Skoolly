...
def get_full_registry_status() -> Dict[str, Any]:
    ...
    for s in schools:
        ...
        current_url = db_s.get("official_website_url") or s.get("website") or txt_e.get("url") or ""
        source = db_s.get("website_source") or s.get("website_source") or ("Verified Official Registry" if txt_e.get("url") else "Not Checked")
        
        # Determine verified status
        is_verified = "Verified" in source or (txt_e.get("url") and txt_e.get("url") == current_url)
        if is_verified:
            status = "verified"
            verified_count += 1
        elif "OPEC" in source:
        ...
        items.append({
            "school_code": code,
            "school_name_th": s.get("school_name_th") or db_s.get("name_th") or txt_e.get("name_th") or "",
            "school_name_en": s.get("school_name_en") or db_s.get("name_en") or txt_e.get("name_en") or "",
            ...
            with psycopg.connect(dsn) as conn:
                with conn.cursor() as cur:
                    cur.execute(
                        ...
                        """,
                        {"web": website, "src": source, "code": school_code}
                    )
                    conn.commit()
        except Exception as e:
            print("[Website Registry] Error updating Supabase:", e)

    # 3. Update schoolAndURL.txt
    txt_entries = read_school_and_url_txt()
