...
def extract_coordinates_from_web_fast(url, province=""):
    if not url: return None, None
    u = url.strip()
    if not u.startswith("http"): u = "https://" + u
    try:
        with session.get(u, timeout=2.0, stream=True, allow_redirects=True) as r:
...
def geocode_arcgis_precision_full(school_name_th, school_name_en, address, district, province, subdistrict=""):
    clean_th = re.sub(r'^(โรงเรียน)?นานาชาติ\s*', '', school_name_th).strip()
    clean_en = str(school_name_en or "").strip()
    
    # -------------------------------------------------------------
    # Tier 1: Full ArcGIS POI Campus Queries
    # -------------------------------------------------------------
    poi_queries = []
    if clean_en:
    ...
    # -------------------------------------------------------------
    # Tier 2: Cleaned Street Address & Soi Variations (ArcGIS)
    # -------------------------------------------------------------
    ...
    # -------------------------------------------------------------
    # Tier 3: Open-Meteo District / Subdistrict Pinpoint (Approximate Fallback)
    # -------------------------------------------------------------
...
def process_school(s):
    ...
    # 1. Map Embed (Fast Stream)
    if web:
        w_lat, w_lon = extract_coordinates_from_web_fast(web, p)
        if w_lat and w_lon:
            return s, w_lat, w_lon, "Website Map Embed", "Exact"

    # 2. Geocode
    lat, lon, src, prec = geocode_arcgis_precision_full(th, en, addr, d, p, sd)
...
print(f"Testing on {len(targets)} schools...")
start = time.time()
exact_count = 0
approx_count = 0
